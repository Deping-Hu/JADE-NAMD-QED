    subroutine sub_tully_hopping_ana_nac_cavity(n_atom, &
                                                mass, &
                                                old_vel_x, &
                                                old_vel_y, &
                                                old_vel_z, &
...
!     ----- Argument -----
       ...
       integer, intent(in) :: n_atom, n_state, ntime_ele, it, n_photon_state, n_state_ele
...
!  Local
       ...
       double precision :: rho_decress_all
...
!   For test purpose
       ...
       complex(kind=8), allocatable, dimension(:, :) :: old_rho_tmp
...
!   For test purpose
       ...
       allocate (old_rho_tmp(n_state, n_state))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
...
!!!! Global Flux Surface Hopping (GFSH)
          ...
          if (label_hop_prob .eq. 2) then

             if (real(rho(index_state, index_state)) .lt. real(old_rho_tmp(index_state, index_state))) then

                rho_decress_all = 0.d0

                do i = 1, n_state
                   if (real(rho(i, i)) .lt. real(old_rho_tmp(i, i))) then
                      rho_decress_all = rho_decress_all + (real(old_rho_tmp(i, i)) - real(rho(i, i)))
                   end if
                end do
                ...
                do i = 1, n_state
                   ...
                   pro_hop(i) = pro_hop(i) &
                                +(real(rho(i, i)) - real(old_rho_tmp(i, i))) &
                                /real(old_rho_tmp(index_state, index_state)) &
                                *(real(old_rho_tmp(index_state, index_state)) - real(rho(index_state, index_state))) &
                                /rho_decress_all
