...
def copy_energy(n_traj_select):
    ...
    for i in range(n_traj_select + 1):
        ...
        if (label_mu == 'YES'):
           command3 = "cp  ../data_prescreening/" + str(
               i) + '/mu_all_time_afterselect.out     ./all/mu_all_time_afterselect.out_' + str(i)
           os.system(command3)
           command4 = 'cat  ./all/mu_all.dat    ./all/mu_all_time_afterselect.out_' + str(
               i) + '      > ./all/mu_tmp.dat  '
           os.system(command4)
           command5 = 'cp  ./all/mu_tmp.dat    ./all/mu_all.dat  '
           os.system(command5)
