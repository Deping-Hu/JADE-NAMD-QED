...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   integer :: n_state, n_state_ele, n_photon_state, ntime_ele
   ...
   namelist /control/ dyn_method, label_ZN, ntime, dtime, &
      ...
      qm_method, n_state, n_state_ele, n_photon_state, md_state_list, i_state, &
      seed_random, cor_dec, label_nac_phase, label_reject_hops, &
      hop_e, label_read_velocity, label_restart, label_hop_prob

   double precision :: gamma0, temperature
   namelist /langevin/ gamma0, temperature

!!!!!!!!! for dynamics inside the cavity only
   integer :: efield_x, efield_y, efield_z, kkr_mu
   double precision :: wc, gc, photon_loss_rate, af_cut
   character(len=20)::Hamitonian_Compoment, label_permanent_dipole, label_initial_basis
   namelist /cavity/ wc, gc, photon_loss_rate, af_cut, Hamitonian_Compoment, label_permanent_dipole, label_initial_basis, &
      efield_x, efield_y, efield_z, kkr_mu
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   if (dyn_method == 21) then
      write (*, *) "read cavity parameters"
      read (file_dynvar_in, nml=cavity)
   end if
...
!                                 n_states, md_state )
   ...
   if ((n_state .gt. 1) .and. &
       (dyn_method .eq. 21)) then
      write (*, *) "Surface Hopping inside the cavity"

      photon_loss_rate = photon_loss_rate/27.2

      call sub_sh_ana_nac_cavity(n_atom, &
                                 i_state, &
